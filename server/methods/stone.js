...
import { ValidCardNumber, ValidExpireMonth, ValidExpireYear, ValidCVV, ValidDocument } from "/lib/api";
import { StoneGatewayApi } from "./stoneGatewayApi";
...
Meteor.methods({
   ...
   */
  "stoneSubmit"(transactionType, cardData, paymentData) {
   ...
   * @return {Object} results normalized
   */
  "stone/payment/capture"(paymentMethod) {
    ...
    const response = StoneGatewayApi.methods.capture.call({
      authorizationId,
      amount
    });
    const result = {
   ...
   */
  "stone/refund/create"(paymentMethod, amount) {
    check(amount, Number);
    ...
    const response = StoneGatewayApi.methods.refund.call({
      transactionId,
      amount
    });
    const results = {
   ...
   * @return {Object} result
   */
  "stone/refund/list"(paymentMethod) {
    // Call both check and validate because by calling `clean`, the audit pkg
    // thinks that we haven't checked paymentMethod arg
    check(paymentMethod, Object);
    PaymentMethodArgument.validate(PaymentMethodArgument.clean(paymentMethod));
    ...
    const { transactionId } = paymentMethod;
    const response = StoneGatewayApi.methods.refunds.call({
      transactionId
    });
    const result = [];
